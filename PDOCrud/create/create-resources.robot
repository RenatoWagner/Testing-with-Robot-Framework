...
${back_button}                              class=pdocrud-back
${cancel_button}                            class=pdocrud-cancel-btn
#Checkbox
...
Checking if the mandatory message for First Name field just Appeared
    Textfield Should Contain    ${first_name_field}    Preencha este campo.    
       

Return to the list page
    Wait Until Page Contains Element    ${back_button}    timeout=90
    Click Button    ${back_button}
    
Canceling and cleaning fields
    Wait Until Page Contains Element    ${cancel_button}    timeout=90
    Click Button    ${cancel_button}

    Wait Until Page Contains Element    ${first_name_field}    timeout=90
    Textfield Value Should Be    ${first_name_field}    ${EMPTY}    
    Wait Until Page Contains Element    ${last_name_field}    timeout=90
    Textfield Value Should Be    ${last_name_field}    ${EMPTY}
    Wait Until Page Contains Element    ${adress_field}    timeout=90
    Textfield Value Should Be    ${adress_field}    ${EMPTY}
    Wait Until Page Contains Element    ${city_field}    timeout=90
    Textfield Value Should Be    ${city_field}    ${EMPTY}
    Wait Until Page Contains Element    ${state_field}    timeout=90
    Textfield Value Should Be    ${state_field}    ${EMPTY}
    Wait Until Page Contains Element    ${Zip_field}    timeout=90
    Textfield Value Should Be    ${Zip_field}    ${EMPTY}
